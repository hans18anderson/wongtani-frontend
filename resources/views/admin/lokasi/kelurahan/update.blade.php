...
<div class="row">
  <div class="col-md-12 grid-margin stretch-card">
    <div class="card">
      <div class="card-body">
        <h6 class="card-title">Update Lokasi Kelurahan</h6>
        <a href="/AdminLokasi/kelcon"> Kembali</a>
        <br />
        <form action="/AdminLokasi/updateskelcon" method="post">
        {{ csrf_field() }}
        <input type="hidden" name="id4" class="form-control" id="exampleInputText1" value="{{$data->id}}" placeholder="Enter Status">
          ...
          <div class="form-group">
          <label for="exampleFormControlSelect2">Nama Kecamatan</label>
            <select multiple class="form-control" name="id2" id="exampleFormControlSelect2">
              @foreach($kecdata as $p)
                <option value="{{$p->id}}" @if($p->id == $data->lokasi_kecamatan_id) selected @endif >{{$p->nama}}</option>
              @endforeach
            </select>
          </div>
